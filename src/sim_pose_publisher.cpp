...
void pose_callback(const gazebo_msgs::ModelStates &msg) {
  ...
  tform->setOrigin(
      tf::Vector3(msg.pose[1].position.x, msg.pose[1].position.y, 0));
  tf::Quaternion q;
  q.setRPY(0, 0, tf::getYaw(msg.pose[1].orientation));
  tform->setRotation(q);
  *tform = tform->inverse();
  br->sendTransform(
      tf::StampedTransform(*tform, ros::Time::now(), "base_footprint", "map"));
}
...
int main(int argc, char **argv) {
  ...
  pose_publisher = n.advertise<geometry_msgs::PoseWithCovarianceStamped>(
      "/indoor_pos", 1, true);
